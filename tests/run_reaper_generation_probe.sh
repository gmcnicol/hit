...
probe=$PWD/tests/reaper_generation_probe.lua
output=/tmp/hit-reaper-generation-probe.txt
expected=/tmp/hit-reaper-generation-probe.expected
project=/tmp/hit-reaper-generation-probe.rpp
run_id=$$
...
run_phase() {
  ...
  local marker=$2
  shift 2
  local log=/tmp/hit-reaper-generation-$mode.log

  HIT_GENERATION_PROBE_MODE=$mode HIT_GENERATION_PROBE_RUN_ID=$run_id \
